    ...
    public class Project
    {
        ...
        public void LoadProject(int project_id, bool load_basic = false)
        {
            ...
            DataSet ds = db.Select("SELECT project_id, project_name, "
                                 ...
                                 + "project_filming_date_obs, project_idusuario, project_development_lab_info, project_preprint_store_info, "
                                 ...
                                 + ",obs_adicional_obra,obs_adicional_productor,obs_adicional_otros_prd,obs_adicional_personal,obs_adicional_finalizacion,carta_aclaraciones_generada "
                                 + " FROM project "
        ...
        public bool Save(bool basico=false)
        {
            ...
            if (this.project_id.ToString() != "" && this.project_id > 0)
            {
                ...
                updateProject = updateProject + "obs_adicional_productor = '" + obs_adicional_productor.Replace("'", "´") + "' ,";
                updateProject = updateProject + "obs_adicional_otros_prd = '" + obs_adicional_otros_prd.Replace("'", "´") + "', ";
                updateProject = updateProject + "obs_adicional_personal = '" + obs_adicional_personal.Replace("'", "´") + "' ,";
                updateProject = updateProject + "obs_adicional_finalizacion = '" + obs_adicional_finalizacion.Replace("'", "´") + "' ,";
                updateProject = updateProject + "carta_aclaraciones_generada = '" + carta_aclaraciones_generada.Replace("'", "´") + "' ";                
                updateProject = updateProject + " WHERE project_id = " + this.project_id.ToString();
                
                bool execution=db.Execute(updateProject);
                if (basico)
                {
                    return execution;
                }
                    ...
                    foreach (Producer producer in this.producer)
                    {
                        ...
                        if (saveProducer)
                        {
                            if (producer.Save(this.project_id) && saveProducer)
                            {
                                ...
                                addProducer = addProducer + "VALUES ('" + producer.producer_id + "','" + this.project_id + "','" + producer.participation_percentage + "','" + producer.requester + "')";
        ...
        public void GetProducers()
        {
            ...
            if (this.project_id.ToString() != "" && this.project_id > 0)
            {
                ...
                for (int i = 0; i < queryProducersDS.Tables[0].Rows.Count; i++)
                {
                    ...
                    newProducer.participation_percentage = queryProducersDS.Tables[0].Rows[0]["project_producer_participation_percentage"].ToString() != "" ? (int)queryProducersDS.Tables[0].Rows[i]["project_producer_participation_percentage"] : 0;
                        ...
                        <table align='center' border='0' cellpadding='0' cellspacing='0' width='600'>
							<tbody>
								<tr>
									...
									Dirección de Cinematografía <br>
        ...
        public bool ValidateProjectSection(string section,bool emptyform = false)
        {
            ...
            switch(section)
            {
                ...
                case "DatosProyecto":
                    ...
                    {
                        this.section_validation_result = this.section_validation_result + "<li>El porcentage no es valido</li>";
                        verifycont++;
                    }
                    ...
                    {
                        this.section_validation_result = this.section_validation_result + "<li>El porcentage para producción debe ser mínimo 51%</li>";
                        verifycont++;
                    }
                    cont++;
                    ...
                    {
                        this.section_validation_result = this.section_validation_result + "<li>El porcentage para coproducción debe ser mínimo 51% e inferior a 100%</li>";
                        verifycont++;
                    }
                ...
                case "DatosProductor":
                    ...
                    foreach (Producer producer in this.producer)
                    {
                        if (producer.requester == 1)
                        {
                            ...
                            cont++;
                            if (producer.producer_identification_number == "")
                            {
                            ...
                            cont++;
                            if (producer.producer_nit == "" && producer.person_type_id == 2)
                            {
                                this.section_validation_result = this.section_validation_result + "<li>No se han registrado el nit de la empresa</li>";
                                verifycont++;
                            }
                            cont++;
...
//si es produccion y solo tiene un productor nacional no debe ingresar nada en este tab
                        ...
                        if (producer.requester == 0)
                        {
                            ...
                            if (producer.producer_id > 0)
                            {
                                ...
                                if (producer.producer_type_id == 1)
                                {
                                    if (producer.producer_nit == "" && producer.person_type_id == 2)
                                    {
                                        local_validation_result = local_validation_result + "<li>No se ha registrado el NIT</li>";
                                        verifycont++;
                                    }
                                }
                                cont++;
                ...
                case "DatosPersonal": //Validación de todas las opciones de personal
                    if (this.project_has_domestic_director < 0) //Valida si la solicitud ya tiene definido si tiene director colombiano o no
                    {
                        this.section_validation_result = "<li>No se ha definido si la obra tiene director nacional</li>";
                        verifycont++;
                    }
                    else if (this.project_staff_option_id <= 0) //Si tiene director colombiano valida si ya tiene definida una opción del personal posible
                    {
                    ...
                    else //Si ya tiene seleccionada una de las opciones de personal disponible entonces se procede con la validación de los datos de cada persona regitrada
                    {
                        ...
                        DataSet staffOptionDetailDS = staff.getStaffOptionDetail(opcion_personal);
        ...
        public string renderAttachments(int attachment_id = 0, int project_attachment_id = 0, string attachment_name = "", string project_attachment_path = "", bool showAdvancedForm = false, int project_attachment_approved = 0,string attachment_required ="",int project_state_id=0,int user_role = 0,string nombre_original="",string tooltip="")
        {
            ...
            {
                item = "<div id='contenedorAdjunto' class='required_field' style='min-height:30px;'>" + item + "</div>";
            }
            else {
                item = "<div id='contenedorAdjunto' class='' style='min-height:30px;'>" + item + "</div>";
            }
            return item;
        }
        ...
        public string renderAttachmentsPersonal(int attachment_id = 0, int project_attachment_id = 0, string attachment_name = "", string project_attachment_path = "", bool showAdvancedForm = false, int project_attachment_approved = 0, string attachment_required = "", int index = 0, int project_state_id = 0, int user_role = 0,string nombre_original="")
        {
            ...
            if (project_attachment_id <= 0 || (((project_state_id == 5 && user_role<=1) || (user_role>1 && project_state_id>4)) && project_attachment_approved != 1 && !showAdvancedForm))
            {
                ...
                item = "<div id='contenedorAdjunto2' class='' style='min-height:30px;'>" + item + "</div>";
            }
